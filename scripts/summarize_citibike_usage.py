...
FIXED VERSION: Adds week_part and trips_per_day columns for build_axa_target_windows.py compatibility

Expected parquet layout:
  <PARQUET_DIR>/mode=<mode>/year=<YYYY>/month=<MM>/tripdata.parquet

Writes CSVs into --out-dir:
  - citibike_trips_by_month.csv         (NOW INCLUDES trips_per_day)
  - citibike_trips_by_year.csv
  - citibike_trips_by_dow.csv           (NOW INCLUDES dow_name, week_part, year)
  - citibike_trips_by_hour.csv          (NOW INCLUDES week_part, year)
...
# -----------------------------
# DOW / Hour summaries (FIXED: now includes week_part)
# -----------------------------
def compute_dow_and_hour(
    ...
    """
    Returns:
      df_dow: columns [mode, year, dow, dow_name, week_part, trips]
      df_hr:  columns [mode, year, hour, week_part, trips]
    """
    dow_rows = []
    hr_rows = []
    ...
    for fp in parquet_files:
        ...
        for batch in iter_parquet_batches(fp, columns=[tcol], batch_size=batch_size):
            ...
            # For DOW: just group by dow
            dow_rows.append(
                pd.DataFrame({"mode": mode, "year": year, "dow": dow}).groupby(["mode", "year", "dow"], as_index=False).size()
            )
            
            # For Hour: include dow so we can compute week_part
            hr_rows.append(
                pd.DataFrame({"mode": mode, "year": year, "hour": hr, "dow": dow}).groupby(["mode", "year", "hour", "dow"], as_index=False).size()
            )

    # Process DOW
    if dow_rows:
        df_dow = pd.concat(dow_rows, ignore_index=True).groupby(["mode", "year", "dow"], as_index=False)["size"].sum()
        df_dow = df_dow.rename(columns={"size": "trips"})
        ...
        # Add week_part (weekend = Saturday/Sunday)
        df_dow["week_part"] = df_dow["dow"].apply(lambda x: "weekend" if x >= 5 else "weekday")
        
        df_dow = df_dow.sort_values(["mode", "year", "dow"]).reset_index(drop=True)
    else:
        df_dow = pd.DataFrame(columns=["mode", "year", "dow", "dow_name", "week_part", "trips"])

    # Process Hour
    if hr_rows:
        df_hr = pd.concat(hr_rows, ignore_index=True).groupby(["mode", "year", "hour", "dow"], as_index=False)["size"].sum()
        df_hr = df_hr.rename(columns={"size": "trips"})
        
        # Add week_part based on dow
        df_hr["week_part"] = df_hr["dow"].apply(lambda x: "weekend" if x >= 5 else "weekday")
        
        # Aggregate by mode, year, hour, week_part (sum across days)
        df_hr = df_hr.groupby(["mode", "year", "hour", "week_part"], as_index=False)["trips"].sum()
        df_hr = df_hr.sort_values(["mode", "year", "week_part", "hour"]).reset_index(drop=True)
    else:
        df_hr = pd.DataFrame(columns=["mode", "year", "hour", "week_part", "trips"])
...
def _load_crashes_with_time(nypd_csv: Path) -> pd.DataFrame:
    ...
    if not date_col:
        raise RuntimeError(f"Could not find crash date column in NYPD file: {nypd_csv}")
    ...
    if not lat_col or not lng_col:
        raise RuntimeError(f"Could not find crash lat/lng columns in NYPD file: {nypd_csv}")
...
def _compute_crash_counts_for_stations(
    ...
    try:
        ...
        raise RuntimeError("scikit-learn is required for crash proximity.") from e
...
def build_highlights_markdown(
    ...
    if not df_dow.empty:
        ...
        lines.append(f"## Top {top_n} days-of-week by trips")
    ...
    if not df_hr.empty:
        ...
        lines.append(f"## Top {top_n} hours by trips")
...
def main() -> None:
    ...
    ap.add_argument("--nypd-crash-csv", default=None)
    ap.add_argument("--radii-m", default="250,500")
    ap.add_argument("--station-exposure", choices=["touchpoints", "starts"], default="touchpoints")
    ap.add_argument("--top-n", type=int, default=5)
    ap.add_argument("--no-print", action="store_true")
    ap.add_argument("--no-highlights", action="store_true")
    ap.add_argument("--highlights-md", default=None)
    ...
    # -----------------------------
    # Phase 1: trips by month/year + trips_per_day (FIXED)
    # -----------------------------
    month_rows = []
    for fp in files:
        part = parse_partition(fp)
    ...
    if df_month.empty:
        df_month = pd.DataFrame(columns=["mode", "year", "month", "trips", "trips_per_day"])
    else:
        df_month = df_month.groupby(["mode", "year", "month"], as_index=False)["trips"].sum()
        
        # Add trips_per_day (FIXED: compute days in each month)
        df_month["days_in_month"] = df_month.apply(
            lambda row: pd.Period(f"{int(row['year'])}-{int(row['month'])}").days_in_month,
            axis=1
        )
        df_month["trips_per_day"] = df_month["trips"] / df_month["days_in_month"]
    ...
    # -----------------------------
    # Phase 2: DOW / Hour (FIXED: now includes week_part)
    # -----------------------------
    ...
    if df_st.empty:
        print("WARNING: Station exposure is empty.")
    ...
    # -----------------------------
    # Phase 4: crash proximity (if requested)
    # -----------------------------
    if args.nypd_crash_csv and radii_f:
        ...
        if mode_filter == "jc":
            ...
            print("Saved crash-proxy CSVs (JC mode: zeros).")

        else:
            # NYC mode
            if not nypd_path.exists():
                print(f"WARNING: NYPD file not found: {nypd_path}")
            else:
                try:
                    ...
                    print(f"Saved -> {out_dir / 'station_risk_exposure_plus_crashproximity.csv'}")
                    ...
                    print(f"WARNING: Crash proximity error: {e}")
